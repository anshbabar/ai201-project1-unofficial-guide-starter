...
**Milestone 3 — Ingestion and chunking:** I used Claude to help create the ingestion and chunking pipeline based on my document format and Chunking Strategy section. The pipeline loads all 10 text files, removes unnecessary whitespace, parses metadata and category ratings, extracts the written review, and creates one chunk per review. I verified the output by checking that 10 documents and 10 chunks were created, printing five sample chunks, and confirming that each chunk was readable, complete, and connected to the correct source file. I kept each review as one chunk instead of using a fixed character split because the reviews were short and already represented complete student opinions.

**Milestone 4 — Embedding and retrieval:** I gave Claude my Retrieval Approach, Architecture, and sample chunks and asked for help implementing embeddings with all-MiniLM-L6-v2, storing the chunks in ChromaDB, and retrieving the three most relevant chunks with distance scores. The initial retrieval results were weak for topics such as food and opportunities, so I updated the searchable chunk text to include category ratings, normalized the embeddings, and configured ChromaDB to use cosine distance. I verified the changes by running three evaluation queries, inspecting the retrieved source files and distance scores, and confirming that most results were relevant and below a distance of 0.5. I also changed top_k from four to three to keep the context more focused for a dataset containing only 10 reviews.

**Milestone 5 — Generation and interface:** I gave Claude my grounding requirements, retrieval function, and desired output format and asked for help connecting the pipeline to Groq’s llama-3.3-70b-versatile model and a Gradio interface. The generated pipeline instructs the model to answer only from the retrieved reviews, identify conflicting student opinions, and refuse questions when the reviews do not contain enough information. Source filenames are added programmatically below each answer so attribution does not depend only on the language model. I verified the system by testing supported questions about social life, food, clubs, and opportunities, as well as an unsupported question about UCSD’s acceptance rate, which the system correctly refused to answer.
